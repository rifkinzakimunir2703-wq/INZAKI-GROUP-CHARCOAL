...
$('#dProfitWeek').textContent=rp(ws.reduce((a,x)=>a+x.total,0)-wc-we);$('#dProfitMonth').textContent=rp(ms.reduce((a,x)=>a+x.total,0)-mc-me);$('#dExpenseWeek').textContent=rp(we);$('#dExpenseMonth').textContent=rp(me);
...
$('#salesTable').innerHTML=S.sales.slice().reverse().map(x=>{let b=B(x.batchId),c=x.qty*(b?b.hppkg:0);return `<tr><td>${x.date}</td><td>${b?.code||'-'}</td><td>${esc(x.customer)}</td><td>${kg(x.qty)}</td><td>${rp(x.total)}</td><td>${rp(c)}</td><td>${rp(x.total-c)}</td></tr>`}).join('')||empty(7);
